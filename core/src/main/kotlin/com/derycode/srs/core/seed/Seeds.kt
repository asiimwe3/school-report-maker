...
object Seeds {
    ...
    /** Same subject code (e.g. MATH) legitimately exists at multiple levels — the id must
     *  carry the level too, or two different subjects collide onto one id (Section: data
     *  integrity). Never drop the level tag here even if a level list is edited later. */
    private fun Level.tag(): String = when (this) { Level.PRIMARY -> "p"; Level.O_LEVEL -> "o"; Level.A_LEVEL -> "a" }
    ...
    fun subjectsFor(level: Level): List<Subject> = when (level) {
        ...
        ).map { (n, c) -> Subject(id = "sub-${level.tag()}-${c.lowercase()}", name = n, code = c, level = level, maxMarks = 100) }
        ...
        Level.A_LEVEL -> listOf(
            ...
            Subject(
                id = "sub-${level.tag()}-${c.lowercase()}", name = n, code = c, level = level,
